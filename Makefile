...
build: ## Build necessary image for building packages
	docker-compose -f .docker/docker-compose.yml build abuild

build-packages: ## Usage: make build-package [p="7.0|7.1|7.2|all|<package-name1> <package-name2> ..."]
	make build
	docker-compose -f .docker/docker-compose.yml run --rm abuild build-packages "$(p)"

generate-index: ## Generate index file APKINDEX.tar.gz usage: make generate-index
	make build
	docker-compose -f .docker/docker-compose.yml run --rm abuild generate-index

abuild-generate-private-key: ## Generate new private key
	docker-compose -f .docker/docker-compose.yml run abuild openssl genrsa -out phpearth.rsa.priv 4096 --build --force-recreate

abuild-generate-public-key: ## Generate new public key
	docker-compose -f .docker/docker-compose.yml run abuild openssl rsa -in phpearth.rsa.priv -pubout -out /repo/phpearth.rsa.pub

clean: ## Remove pkg, src, tmp and log folders when building packages for Alpine
	rm -rf build/main/*/pkg build/main/*/src build/main/*/tmp log/*

sh: ## Run shell
	docker-compose -f .docker/docker-compose.yml run --rm abuild sh
