...
run: ## Run a command in a new Docker container; make run a=[...]
  make build
  @docker container run -it --rm -v `pwd`/packages:/packages -v `pwd`/public:/public phpearth-abuild:$(version) $(a)
...
upload: ## Upload build packages to Linux repos server
  @rsync -avz --del public/packages/ repos.php.earth:~/repos/alpine/v$(version)/
